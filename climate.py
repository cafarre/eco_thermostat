...
CONF_PROG_TEMP = "prog_temp"
CONF_PROG_TEMP_HOLIDAY = "prog_temp_holidays"
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        ...
        #New fields
        vol.Optional(CONF_TIMER): cv.entity_id,
        vol.Optional(CONF_MAX_TIME_ON): cv.positive_time_period,
        vol.Optional(CONF_RESTART_DELAY): cv.positive_time_period,
        vol.Optional(CONF_CALENDAR_HOLIDAYS): cv.entity_id,
        vol.Optional(CONF_MAX_TEMP_JUMPS, default=[]): vol.All(
            cv.ensure_list, [cv.string]),
        vol.Optional(CONF_PROG_TEMP, default=[]): vol.All(
            cv.ensure_list, [cv.string]),
        vol.Optional(CONF_PROG_TEMP_HOLIDAY, default=[]): vol.All(
            cv.ensure_list, [cv.string]),
...
async def async_setup_platform(
    ...
    #new fields
    timer = config.get(CONF_TIMER)
    max_time_on = config.get(CONF_MAX_TIME_ON)
    restart_delay = config.get(CONF_RESTART_DELAY)
    calendar_holidays = config.get(CONF_CALENDAR_HOLIDAYS)
    max_temp_jumps = config.get(CONF_MAX_TEMP_JUMPS)
    prog_temp = config.get(CONF_PROG_TEMP)
    prog_temp_holidays = config.get(CONF_PROG_TEMP_HOLIDAY)
